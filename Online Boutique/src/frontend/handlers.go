...
func (fe *frontendServer) productHandler(w http.ResponseWriter, r *http.Request) {
	...
	var inventoryInfo *InventoryInfo = nil
	quantityOptions := []int{1, 2, 3, 4, 5, 10}
	if isInventoryServiceConfigured() {
		inventoryInfo, err = httpGetInventoryInfo(id)
		if err != nil {
			fmt.Println("Failed to obtain product inventory info:", err)
		} else {
			quantityOptions = availableQuantityOptions(inventoryInfo.Quantity)
		}
	}
...
func (fe *frontendServer) addToCartHandler(w http.ResponseWriter, r *http.Request) {
	...
	if isInventoryServiceConfigured() {
		if _, err := httpReserveInventory(p.GetId(), payload.Quantity); err != nil {
			renderHTTPError(log, r, w, errors.Wrap(err, "could not reserve inventory"), http.StatusConflict)
			return
		}
	...
	if err := fe.insertCart(r.Context(), sessionID(r), p.GetId(), int32(payload.Quantity)); err != nil {
		if inventoryReserved {
			if _, releaseErr := httpReleaseInventory(p.GetId(), payload.Quantity); releaseErr != nil {
				log.WithField("error", releaseErr).Warn("failed to release reserved inventory")
			}
		}
...
func (fe *frontendServer) emptyCartHandler(w http.ResponseWriter, r *http.Request) {
	...
	if isInventoryServiceConfigured() {
		for _, item := range cart {
			if _, err := httpReleaseInventory(item.GetProductId(), uint64(item.GetQuantity())); err != nil {
				log.WithField("error", err).WithField("product", item.GetProductId()).Warn("failed to release inventory after emptying cart")
			}
		}
	}
...
func (fe *frontendServer) adminRestockPageHandler(w http.ResponseWriter, r *http.Request) {
	log := r.Context().Value(ctxKeyLog{}).(logrus.FieldLogger)
	...
	data := map[string]interface{}{
		"show_currency":       false,
		"admin_authenticated": isAdminAuthenticated(r),
		"login_error":         r.URL.Query().Get("error") == "1",
		"restock_success":     r.URL.Query().Get("updated") == "1",
	}
	...
	if isAdminAuthenticated(r) {
		...
		adminProducts := make([]adminProductView, 0, len(products))
		for _, product := range products {
			inventory, inventoryErr := httpGetInventoryInfo(product.GetId())
			if inventoryErr != nil {
				log.WithField("error", inventoryErr).WithField("product", product.GetId()).Warn("failed to retrieve admin inventory")
			}
			adminProducts = append(adminProducts, adminProductView{Item: product, Inventory: inventory})
		}
		data["admin_products"] = adminProducts
	}

	if err := templates.ExecuteTemplate(w, "admin_restock", injectCommonTemplateData(r, data)); err != nil {
		log.Println(err)
	}
}

func (fe *frontendServer) adminLoginHandler(w http.ResponseWriter, r *http.Request) {
	if !validAdminCredentials(r.FormValue("username"), r.FormValue("password")) {
		w.Header().Set("location", baseUrl+"/admin/restock?error=1")
		w.WriteHeader(http.StatusFound)
		return
	}

	setAdminSession(w)
	w.Header().Set("location", baseUrl+"/admin/restock")
	w.WriteHeader(http.StatusFound)
}
...
func (fe *frontendServer) adminRestockHandler(w http.ResponseWriter, r *http.Request) {
	log := r.Context().Value(ctxKeyLog{}).(logrus.FieldLogger)
	if !isAdminAuthenticated(r) {
		renderHTTPError(log, r, w, errors.New("administrator authentication required"), http.StatusUnauthorized)
		return
	}
	...
	if productID == "" || quantity == 0 {
		renderHTTPError(log, r, w, errors.New("product id and positive quantity are required"), http.StatusBadRequest)
		return
	}

	if !isRestockServiceConfigured() {
		renderHTTPError(log, r, w, errors.New("restock service is not configured"), http.StatusServiceUnavailable)
		return
	}

	if err := httpRestockProduct(productID, quantity); err != nil {
		renderHTTPError(log, r, w, errors.Wrap(err, "failed to restock product"), http.StatusBadGateway)
		return
	}

	w.Header().Set("location", baseUrl+"/admin/restock?updated=1")
	w.WriteHeader(http.StatusFound)
}
...
func injectCommonTemplateData(r *http.Request, payload map[string]interface{}) map[string]interface{} {
	data := map[string]interface{}{
		...
		"admin_enabled":     isRestockServiceConfigured(),
...
func availableQuantityOptions(available int) []int {
	if available <= 0 {
		return []int{}
	}

	defaultOptions := []int{1, 2, 3, 4, 5, 10}
	options := []int{}
